...
def draw(uav, sensors, details=False):
    ...
    ax.plot([params.length_range, params.length_range],
            [0, params.length_range], c='#000000')
    ax.plot([0, params.length_range], [
            params.length_range, params.length_range], c='#000000')
    ...
    for record in uav.records:
        ...
        if (current_x, current_y) == (0.0, 0.0):
            ...
            if details:
                if tour > 1:
                    fig.savefig(
                        './out/{:%m-%d-%H-%M-%S}_{}.png'.format(params.time, tour - 1))
                plt.close('all')
    ...
    if details:
        fig.savefig(
            './out/{:%m-%d-%H-%M-%S}_{}.png'.format(params.time, tour))
    else:
